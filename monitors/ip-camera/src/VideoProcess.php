...
class VideoProcess
{
    ...
    public function run()
    {
        ...
        $ffmpeg_command = "ffmpeg -i {$this->path} -c copy -map 0 -acodec {$this->audio_codec} -f segment -strftime 1 -segment_time {$this->segment_time} -segment_format {$this->format} /app/videos/{$this->name}_{$this->timestamp_format}.mp4 ";
